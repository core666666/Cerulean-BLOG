...
public class TagController {
    ...
    public String tagPage(HttpServletRequest request) {
        request.setAttribute("path", "tags");   // 对 siderbar.html 中的 ${path} 进行赋值
        return "admin/tag";  // 返回 admin/tag.html
    }
    ...
    @ResponseBody
    public Result delete(@RequestBody Integer[] ids) {  // 将接受的参数设置为一个数组，可以同时删除多条记录
        if (ids.length < 1) {
            return ResultGenerator.genFailResult("参数异常！");
        }
