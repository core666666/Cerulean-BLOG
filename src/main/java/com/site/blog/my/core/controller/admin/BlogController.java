...
public class BlogController {
    ...
    public String edit(HttpServletRequest request) {
        ...
        // 返回一个 List<BlogCategory> 填充到 edit.html 的 ${categories}
    ...
    /*
        修改文章状态 草稿/发布
     */
    @PostMapping("/blogs/setblogsstatus")
    @ResponseBody
    public Result blogsstatus(@RequestBody Long[] ids) {
        if (ids.length < 1) {
            return ResultGenerator.genFailResult("参数异常！");
        }
        if (blogService.setstatusBatch(ids)) {
            return ResultGenerator.genSuccessResult();
        } else {
